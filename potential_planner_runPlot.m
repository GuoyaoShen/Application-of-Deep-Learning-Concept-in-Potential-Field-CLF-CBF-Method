...
function potential_planner_runPlot(potential,plannerParameters,PATH_world,varargin)
%styles used for each line
styles={'r','g','b','c','m'};
%load world data
% load('sphereworld.mat','xGoal','xStart','world');
load(PATH_world,'xGoal','xStart','world');
% xStart = xStart(:,2);  % self added
% xGoal = xGoal(:,2);  % self added
...
for iGoal=1:size(xGoal,2)
    ...
    for iStart=1:nbStarts
        subplot(1,2,1)
        [xPath, UPath] = potential_planner(xStart(:,iStart), world, potential, plannerParameters,varargin{:});  % passin optimizer setting
        hold on
    ...
    subplot(1,2,2)
    title(['Goal ' num2str(iGoal) ', potential || ' num2str(size(UPath(~isnan(UPath)),2)) 'steps'])
    xlabel('# steps')
    ylabel('U')
end